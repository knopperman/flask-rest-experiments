...
- 🎯 Entry points to execute your program using `python -m <flask_rest_experiments>` or `$ flask_rest_experiments` with basic CLI argument parsing.
...
---
# flask_rest_experiments Flask Application

Awesome flask_rest_experiments created by knopperman
...
```bash
git clone https://github.com/knopperman/flask-rest-experiments flask_rest_experiments
cd flask_rest_experiments
make install
```
...
```bash
pip install flask_rest_experiments
```
...
```bash
$ flask_rest_experiments
```

or

```bash
$ python -m flask_rest_experiments
```
...
```bash
flask_rest_experiments create-db   # run once
flask_rest_experiments populate-db  # run once (optional)
flask_rest_experiments add-user -u admin -p 1234  # ads a user
flask_rest_experiments run
```

...
original_name="flask_rest_experiments"
original_urlname="flask-rest-experiments"
original_description="Awesome flask_rest_experiments created by knopperman" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_rest_experiments "${name}"
...
then
    ...
    echo "then run 'flask_rest_experiments' or 'python -m flask_rest_experiments'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_rest_experiments' or 'python -m flask_rest_experiments' or 'poetry run flask_rest_experiments'"
fi
